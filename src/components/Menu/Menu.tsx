...
function Menu(props: { config: GameConfig; onConfigChanged: (config: GameConfig) => void }) {
  ...
  const {
    ...
    getValues,
    formState: { errors }
  } = useForm<GameConfig>({ mode: 'onChange' });
  ...
  useEffect(() => {
    function handleClickOutside(event: MouseEvent) {
      ...
      if (
        ...
        !target.closest('.menu__new-game-btn')
      ) {
  ...
  return (
    <nav className="menu">
      ...
      {isMenuShown && (
        <form ref={menu} className="config-form" onSubmit={handleSubmit(onSubmit)}>
          <div className="form-input">
            <label className="form-input__label" htmlFor="width">
              Width
            </label>
            <input
              type="number"
            ...
            {errors.width && (
              <span className="form-input__error" role="alert">
                {getErrorMessage(errors.width)}
              </span>
            )}
          </div>
          <div className="form-input">
            <label className="form-input__label" htmlFor="height">
              Height
            </label>
            <input
              type="number"
            ...
            {errors.height && (
              <span className="form-input__error" role="alert">
                {getErrorMessage(errors.height)}
              </span>
            )}
          </div>
          <div className="form-input">
            <label className="form-input__label" htmlFor="bombs">
              Bombs
            </label>
            <input
              type="number"
              ...
              {...register('bombs', {
                ...VALIDATORS.bombs,
                validate: () =>
                  getValues('bombs') < getValues('width') * getValues('height')
                    ? true
                    : `The field should be less than ${getValues('width') * getValues('height')}`,
                valueAsNumber: true
              })}
