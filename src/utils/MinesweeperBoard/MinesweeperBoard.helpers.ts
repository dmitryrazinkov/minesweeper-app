 ...
 */
export function calcBombsAround(
  xCoord: number,
  yCoord: number,
  width: number,
  height: number,
  bombsLocations: TypedFastBitSet
): number {
 ...
 * @param {number} yCoord - y coordinate of a cell.
 * @param {number} width - The width of a board.
 * @param {TypedFastBitSet} openedCells - The bitset with opened cells locations.
 ...
 * @param {number} yCoord - y coordinate of a cell.
 * @param {number} width - The width of a board.
 * @param {TypedFastBitSet} flaggedCells - The bitset with flagged cells locations.
 * @returns {boolean} - true if flagged, false if not flagged.
 */
export function isFlagged(
  xCoord: number,
  yCoord: number,
  width: number,
  flaggedCells: TypedFastBitSet
) {
  const positionInBitSet = xCoord + yCoord * width;

  return flaggedCells.has(positionInBitSet);
}
